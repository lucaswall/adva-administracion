 ...
 */
export function extractDniFromCuit(cuit) {
    if (!cuit)
        return '';
    // Remove dashes and spaces
    const cleaned = cuit.replace(/[-\s]/g, '');
 ...
 */
export function cuitContainsDni(cuit, dni) {
    if (!cuit || !dni)
        return false;
 ...
 */
export function cuitOrDniMatch(id1, id2) {
    if (!id1 || !id2)
        return false;
 ...
 */
export function isValidCae(cae) {
    if (!cae)
        return false;
...
export function validateFactura(data) {
    ...
    if (!data.tipoComprobante)
        errors.push('Missing tipoComprobante');
    if (!data.puntoVenta)
        errors.push('Missing puntoVenta');
    if (!data.numeroComprobante)
        errors.push('Missing numeroComprobante');
    if (!data.fechaEmision)
        errors.push('Missing fechaEmision');
    if (!data.cuitEmisor)
        errors.push('Missing cuitEmisor');
    else if (!isValidCuit(data.cuitEmisor))
        errors.push('Invalid cuitEmisor');
    if (!data.razonSocialEmisor)
        errors.push('Missing razonSocialEmisor');
    if (!data.cae)
        errors.push('Missing cae');
    else if (!isValidCae(data.cae))
        errors.push('Invalid cae');
    if (!data.fechaVtoCae)
        errors.push('Missing fechaVtoCae');
    if (data.importeNeto === undefined)
        errors.push('Missing importeNeto');
    if (data.importeIva === undefined)
        errors.push('Missing importeIva');
    if (data.importeTotal === undefined)
        errors.push('Missing importeTotal');
    if (!data.moneda)
        errors.push('Missing moneda');
...
export function validatePago(data) {
    ...
    // Required fields
    if (!data.banco)
        errors.push('Missing banco');
    if (!data.fechaPago)
        errors.push('Missing fechaPago');
    if (data.importePagado === undefined)
        errors.push('Missing importePagado');
 ...
 * @returns Validation result with errors
 */
export function validateRecibo(data) {
    const errors = [];
    // Required fields
    if (!data.tipoRecibo)
        errors.push('Missing tipoRecibo');
    if (!data.nombreEmpleado)
        errors.push('Missing nombreEmpleado');
    if (!data.cuilEmpleado)
        errors.push('Missing cuilEmpleado');
    else if (!isValidCuit(data.cuilEmpleado))
        errors.push('Invalid cuilEmpleado');
    if (!data.legajo)
        errors.push('Missing legajo');
    if (!data.cuitEmpleador)
        errors.push('Missing cuitEmpleador');
    else if (!isValidCuit(data.cuitEmpleador))
        errors.push('Invalid cuitEmpleador');
    if (!data.periodoAbonado)
        errors.push('Missing periodoAbonado');
    if (!data.fechaPago)
        errors.push('Missing fechaPago');
    if (data.subtotalRemuneraciones === undefined)
        errors.push('Missing subtotalRemuneraciones');
    if (data.subtotalDescuentos === undefined)
        errors.push('Missing subtotalDescuentos');
    if (data.totalNeto === undefined)
        errors.push('Missing totalNeto');
    return {
        valid: errors.length === 0,
        errors
    };
 ...
 */
export function validateTipoComprobante(value) {
    if (typeof value !== 'string')
        return undefined;
    const validTypes = ['A', 'B', 'C', 'E', 'NC', 'ND'];
 ...
 */
export function validateMatchConfidence(value) {
    if (typeof value !== 'string')
        return undefined;
    const validLevels = ['HIGH', 'MEDIUM', 'LOW'];
    return validLevels.includes(value) ? value : undefined;
}
/**
 ...
 */
export function validateMoneda(value) {
    if (typeof value !== 'string')
        return undefined;
    const validCurrencies = ['ARS', 'USD'];
    return validCurrencies.includes(value) ? value : undefined;
}
/**
 ...
 */
export function validateTipoRecibo(value) {
    if (typeof value !== 'string')
        return undefined;
    const validTypes = ['sueldo', 'liquidacion_final'];
    return validTypes.includes(value) ? value : undefined;
}
