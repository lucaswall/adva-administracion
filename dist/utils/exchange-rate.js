 ...
 */
export function setExchangeRateCache(date, rate) {
 ...
 */
function setCachedValue(key, rate) {
    memoryCache.set(key, {
        rate,
        timestamp: Date.now()
    });
 ...
 */
export function getExchangeRateSync(date) {
    const isoDate = normalizeDateToIso(date);
    if (!isoDate) {
        return {
            ok: false,
 ...
 */
export async function prefetchExchangeRates(dates) {
    const uniqueDates = [...new Set(dates.map(d => normalizeDateToIso(d)).filter(Boolean))];
 ...
 */
export function amountsMatchCrossCurrency(facturaAmount, facturaMoneda, facturaFecha, pagoAmount, tolerancePercent) {
