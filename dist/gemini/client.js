...
export class GeminiClient {
     ...
     */
    async analyzeDocument(fileBuffer, mimeType, prompt, maxRetries = 3) {
     ...
     * @returns API response text or error
     */
    async doAnalyzeDocument(fileBuffer, mimeType, prompt) {
        try {
            ...
            return {
                ok: false,
                error: new GeminiError(error instanceof Error ? error.message : 'Unknown error', undefined, error)
            };
        }
    }
    /**
     ...
     */
    buildApiRequest(prompt, base64Data, mimeType) {
        return {
            contents: [{
                    role: 'user',
                    parts: [
     ...
     */
    parseApiResponse(responseText, statusCode) {
        if (statusCode !== 200) {
            return {
                ok: false,
                error: new GeminiError(`API returned status ${statusCode}`, statusCode, responseText)
            };
        }
        let parsedResponse;
        try {
            ...
            return {
                ok: false,
                error: new GeminiError('Failed to parse API response as JSON', undefined, responseText)
            };
        }
        if (parsedResponse.error) {
            return {
                ok: false,
                error: new GeminiError(parsedResponse.error.message, parsedResponse.error.code, parsedResponse.error)
            };
        }
