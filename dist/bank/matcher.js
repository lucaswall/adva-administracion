 ...
 */
export function isDirectDebit(concepto) {
    if (!concepto) {
        return false;
    }
 ...
 */
export function extractKeywordTokens(concepto) {
    if (!concepto) {
        return [];
    }
 ...
 */
export function calculateKeywordMatchScore(bankConcepto, emisorName, facturaConcepto) {
 ...
 */
export function isCreditCardPayment(concepto) {
    if (!concepto) {
        return false;
    }
 ...
 */
export function isBankFee(concepto) {
    if (!concepto) {
        return false;
    }
 ...
 */
export function extractCuitFromConcepto(concepto) {
    if (!concepto) {
        return undefined;
    }
...
export class BankMovementMatcher {
     ...
     */
    constructor(crossCurrencyTolerancePercent = DEFAULT_CROSS_CURRENCY_TOLERANCE) {
     ...
     */
    findMatchingPagos(amount, bankFecha, bankFechaValor, extractedCuit, pagos) {
        const matches = [];
     ...
     */
    findMatchingFacturas(amount, bankFecha, bankFechaValor, extractedCuit, bankConcepto, facturas) {
        const matches = [];
        ...
        for (const factura of facturas) {
            ...
            const crossCurrencyResult = amountsMatchCrossCurrency(factura.importeTotal, factura.moneda, factura.fechaEmision, amount, this.crossCurrencyTolerancePercent);
            ...
            if (!hasCuit && isDirectDebitMovement) {
                keywordScore = calculateKeywordMatchScore(bankConcepto, factura.razonSocialEmisor, factura.concepto);
        ...
        return matches.map(m => ({
            factura: m.factura,
            reasons: m.reasons,
            matchType: m.hasCuit ? 'cuit' : 'keyword'
        }));
    }
    /**
     * Finds recibos matching amount and date criteria
     */
    findMatchingRecibos(amount, bankFecha, bankFechaValor, recibos) {
        const matches = [];
     ...
     */
    createPagoFacturaMatch(movement, _pago, factura, extractedCuit, reasons) {
     ...
     */
    createDirectFacturaMatch(movement, factura, extractedCuit, reasons, facturaMatchType = 'cuit') {
        const description = this.formatFacturaDescription(factura);
     ...
     */
    createReciboMatch(movement, recibo, extractedCuit, reasons) {
     ...
     */
    createPagoOnlyMatch(movement, pago, extractedCuit, reasons) {
     ...
     */
    createCreditCardPaymentMatch(movement) {
        return {
            movement,
     ...
     */
    formatFacturaDescription(factura) {
