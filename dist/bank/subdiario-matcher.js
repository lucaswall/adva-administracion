 ...
 */
export function extractCuitFromMovementConcepto(concepto) {
    if (!concepto) {
        return undefined;
    }
 ...
 */
function amountsMatchWithTolerance(amount1, amount2) {
...
export class SubdiarioMatcher {
     ...
     */
    matchMovement(movement, cobros, usedCobros) {
        ...
        if (dateMatch) {
            ...
            const confidence = days <= MEDIUM_CONFIDENCE_DAYS ? 'MEDIUM' : 'LOW';
            return {
                matched: true,
     ...
     */
    findCuitMatch(amount, cuit, movementDate, cobros) {
        let bestMatch = null;
     ...
     */
    findDateMatch(amount, movementDate, cobros) {
        let bestMatch = null;
        for (const cobro of cobros) {
            // Check amount
            if (!amountsMatchWithTolerance(cobro.total, amount)) {
