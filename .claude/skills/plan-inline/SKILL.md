...
- **Flag migration-relevant tasks** — If a task changes spreadsheet schema, renames columns, changes folder structure, renames env vars, or changes API response shapes, add a note in the task: "**Migration note:** [what production data is affected and how to migrate]". The plan MUST include a migration strategy (e.g., startup detection of old format + automatic migration). The implementer will log this in `MIGRATIONS.md`.
