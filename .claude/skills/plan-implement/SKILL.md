...
See [references/worktree-setup.md](references/worktree-setup.md) for the full procedure: feature branch detection, prior-run cleanup, worktree creation (hyphen-not-slash naming), `.gitignore` symlink pre-check, dep bootstrap (symlink rationale vs copy), and setup-failure fallback.
...
Merge worker branches into the feature branch **one at a time, foundation-first** (types/schemas → services → API routes → utilities; same-layer workers by number). Run `npm run typecheck` after each merge starting from the second. See [references/merge-procedures.md](references/merge-procedures.md) for full ordering rules, conflict resolution, and cherry-pick fallback (including worktree-symlink artifact recovery).
