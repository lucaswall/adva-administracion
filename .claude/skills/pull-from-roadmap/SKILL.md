...
- **Patterns** — What conventions and patterns are established? (Result<T,E>, Pino logger, CellDate/CellNumber, etc.)
- **Tests** — What test coverage exists in related areas?
- **Dependencies** — What libraries/APIs are already in use that relate? (Google Drive/Sheets, Gemini, Fastify)
...
- **User context relevance** — Use project context from CLAUDE.md (Argentine invoicing, single-user admin tool, backend-only API, Google Sheets persistence) to evaluate feasibility
...
[Honest assessment of whether this feature is feasible, practical, and valuable. Include Argentine-specific, technical, or cost concerns.]
...
- **User's context matters** — Use project context from CLAUDE.md (Argentine invoicing, single-user admin tool, Google Sheets persistence, Railway deployment) to inform the analysis
