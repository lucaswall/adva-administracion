...
Three branches, each with an exact markdown template. See [references/findings-templates.md](references/findings-templates.md) for full templates:

- **Issues found and fixed inline** — append "Review Findings" with "fixed inline" summary, Linear Updates (Review→Merge + new issues in Merge), Inline Fix Verification, `<!-- REVIEW COMPLETE -->`. No Fix Plan.
- **Issues found (creating Fix Plan)** — append "Review Findings" with FIX/DISCARDED summary, Linear Updates (Review→Merge + new issues in Todo), `<!-- REVIEW COMPLETE -->`, then `## Fix Plan` at h2 with one `### Fix N:` section per bug (each with Linear Issue link + TDD steps).
- **No issues found (or all discarded)** — append "Review Findings" stating no bugs, Linear Updates (all Review→Merge), `<!-- REVIEW COMPLETE -->`.
...
- **Codex findings must be assessed for validity** — The monitor never accepts a Codex finding on face value. Each finding is verified against the actual code, accepted patterns, and CLAUDE.md before fixing. Invalid findings are resolved with reasoning, not silently dismissed.
