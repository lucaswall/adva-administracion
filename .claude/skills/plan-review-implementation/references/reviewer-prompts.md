...
- Timeout/hang scenarios — HTTP requests without timeout, external API calls that could hang (Gemini, Railway)
...
- Import path conventions (ESM .js extensions)
- Naming conventions (files: kebab-case, etc.)
- Error response format compliance (Result<T,E> pattern)
- Pino logger usage (no console.log)
- Any other project-specific rules

LOGGING:
- console.log/warn/error instead of Pino logger (check CLAUDE.md for exceptions)
