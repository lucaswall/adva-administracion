...
1. Fetch all Backlog issues: `mcp__linear__list_issues` with `team: "ADVA Administracion"`, `state: "Backlog"`, `includeArchived: false`
...
5. Present to user using `AskUserQuestion`:
   - Include the score and drop indicator in each option's description (e.g., "Match movimientos detalles for pago_recibido — Improvement — Score: 3/5 — Drop? No")
...
|-----------|---------|----------|
| **Likely** | Probably should cancel | Title describes a theoretical concern, "nice to have" with no clear impact, duplicates another issue, or addresses a non-problem for this project context (single-user admin tool, backend-only API) |
...
- **Missing context** — Are affected files, routes, or services identified?
- **Missing impact** — Is it clear why this matters (data quality, processing accuracy, operational impact)?
...
- [Gap 2: e.g., "Description says 'improve matching' but doesn't specify which matcher or scenario"]
- [Gap 3: e.g., "Priority is Low but this affects data accuracy — should be High"]

**Suggested improvements:**
- [Suggestion 1: e.g., "Add specific matching scenarios to handle"]
- [Suggestion 2: e.g., "Split into two issues: factura matching vs pago matching"]
- [Suggestion 3: e.g., "Bump priority to High (Bug label)"]

**Questions for you:**
- [Question 1: e.g., "Should this cover both ARS and USD matching, or just ARS?"]
- [Question 2: e.g., "Is there a specific document that triggered this issue?"]
```
...
- **Not applicable** — Theoretical concern that doesn't apply (e.g., multi-user concurrency issue for a single-user admin tool)
...
**Reasons:**
- [Reason 1: e.g., "This is a single-user admin tool — the race condition described requires concurrent users"]
- [Reason 2: e.g., "The referenced code in src/processing/scanner.ts was refactored in a recent PR"]
...
2. **Apply updates immediately:**
   ...
   - Use `mcp__linear__create_issue` for each new split issue with `team: "ADVA Administracion"`, `state: "Backlog"`, and proper labels/priority
...
- "Problem doesn't exist — the referenced code in src/processing/scanner.ts was refactored."
- "Single-user admin tool — the described race condition requires concurrent users."
- "Cost exceeds value — would require rewriting the entire matching module for a marginal improvement."
...
```
mcp__linear__list_issue_statuses(team: "ADVA Administracion")
```
...
**Context:**
[Affected files, routes, or services — be specific]

**Impact:**
[Why this matters — data accuracy, processing reliability, operational efficiency, etc.]
