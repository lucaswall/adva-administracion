---
name: plan-backlog
description: Convert Linear Backlog issues into TDD implementation plans. Use when user says "plan ADV-123", "plan all bugs", "work on backlog", or wants to implement issues from Linear. With no arguments, plans ALL Backlog issues. Moves planned issues to Todo state. Explores codebase for patterns and discovers available MCPs from CLAUDE.md.
argument-hint: [issue-selector] (optional) e.g., "ADV-123", "all Bug issues" — omit to plan all Backlog items
allowed-tools: Read, Edit, Write, Glob, Grep, Agent, Bash, mcp__linear__list_teams, mcp__linear__list_issues, mcp__linear__get_issue, mcp__linear__save_issue, mcp__linear__save_comment, mcp__linear__list_issue_labels, mcp__linear__list_issue_statuses
...
**If user specified a specific issue (e.g., "ADV-123"):**

```
mcp__linear__get_issue(id: "ADV-123")
```

Verify the issue exists and is in the "Backlog" state. If not in Backlog, warn the user but continue if they confirm.

**If user specified a filter (e.g., "all Bug issues", "the CUIT issue"):**
...
- Available MCP servers and their capabilities (Google Drive, Railway, Gemini, Linear)
...
- **Google Drive MCP**: Document access, spreadsheet schemas
- **Gemini MCP**: Prompt testing for extraction issues

Query relevant MCPs to gather context that will inform the plan. For example:
- Check Drive for document formats relevant to the issues
...
```
mcp__linear__save_comment(issueId: "ADV-xxx", body: "Canceled during triage: [reason]")
```
...
- ADV-123: [title] — [brief reason it's valid]
- ADV-456: [title] — [brief reason it's valid]

**Canceled:**

| Issue | Title | Reason |
|-------|-------|--------|
| ADV-789 | [title] | [specific reason it's invalid] |
| ADV-012 | [title] | [specific reason it's invalid] |
```
...
|-------|-----------------|-------------------|
| **Error handling** | Each task touching Gemini, Google APIs, or Drive has error handling specs | Plan says "call Gemini" with no catch or error behavior spec |
| **Timeouts** | Any Gemini or Google API call has a timeout value specified | Plan says "read from Drive" with no timeout spec |
| **Rate limits** | Google API calls note rate limit handling (exponential backoff, retry budget) | Plan adds new Sheets writes with no rate limit consideration |
| **Edge cases** | Empty input, null values, partial results, and missing documents are addressed | Plan has no test for empty Drive folder or null Gemini response |
| **Conventions** | Result<T,E> pattern, Pino logging, ESM imports, and naming match CLAUDE.md | Plan uses `console.log` instead of Pino logger |
| **Spreadsheet writes** | Any write to spreadsheets specifies behavior on partial failure | Plan adds rows without specifying what happens if write fails mid-batch |
...
**Source field:** `Backlog: ADV-123, ADV-456` (list the issue keys being planned)
...
- Move issues to "Review" when implementation is complete (Done is set automatically by the Linear GitHub integration when the PR merges)
- Add comments to issues with progress updates if needed

### File/Document MCPs (Google Drive)
- Access document formats for extraction-related tasks
- Read spreadsheet schemas for data processing tasks

### Deployment MCPs (Railway)
- Check existing services and their configuration
- Verify environment variables are set correctly
- Understand deployment pipeline for the project

### AI/LLM MCPs (Gemini)
- Test prompt variations for extraction accuracy
- Validate prompt improvements before implementation
