...
7. **Get Linear issue IDs**
   - **If the caller provided explicit issue IDs** (e.g., "Linear issues to close: ADV-123, ADV-124"): use those directly. This is the authoritative list — do NOT scan PLANS.md.
   - **Otherwise:** Read PLANS.md if present in project root. Scan the ENTIRE file for all issue identifiers matching the pattern `ADV-\d+` — not just specific lines. Issues appear in headers, Fix Plans, Linear Updates, task lists, everywhere. Deduplicate and sort numerically.
...
- Comma-separate multiple issues: `Closes ADV-123, ADV-124, ADV-125`
...
- **Fallback to PLANS.md:** Only if no explicit list was provided, scan the ENTIRE PLANS.md for all issue identifiers matching `ADV-\d+` — headers, Fix Plans, Linear Updates, task lists, everywhere. Deduplicate and sort numerically.
