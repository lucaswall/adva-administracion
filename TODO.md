...
## item #1 [security]
Missing resourceId validation in webhook endpoint at src/routes/webhooks.ts:54,76. The webhook extracts resourceId from headers but only validates channelId exists in activeChannels without verifying resourceId matches the expected value. An attacker with a valid channelId could send forged notifications with arbitrary resourceId.

## item #2 [security]
Webhook endpoint lacks rate limiting at src/routes/webhooks.ts:50. The public /webhooks/drive endpoint has no rate limiting. An attacker with a valid channelId could flood the endpoint with notifications triggering excessive scans and causing denial of service.

## item #3 [security]
Empty API_SECRET allowed in development at src/config.ts:92-96. In development and test modes, API_SECRET can be empty string. If deployed with NODE_ENV=development in production, all protected endpoints become accessible without authentication.

## item #4 [security]
No input sanitization in sheet operations at src/processing/storage/factura-store.ts:157,181 and other store files. User-controlled data (factura.concepto, cuitReceptor, etc.) passed directly to spreadsheet without sanitization could potentially inject formulas.

## item #5 [security]
Dependency vulnerability: Hono IPv4 address validation bypass in IP Restriction Middleware (hono@< 4.11.7, GitHub Alert #1). The IPV4_REGEX pattern and convertIPv4ToBinary function do not properly validate that IPv4 octet values are within 0-255 range. Attackers can craft malformed IPs like 1.2.2.355 to bypass IP-based access controls by exploiting overflow behavior during binary conversion. Upgrade to hono >= 4.11.7.

## item #6 [security]
Dependency vulnerability: Hono cache middleware ignores "Cache-Control: private" (hono@< 4.11.7, GitHub Alert #2). Cache Middleware does not respect standard cache control headers like Cache-Control: private or Cache-Control: no-store. May result in private or authenticated responses being cached and exposed to unauthorized users, leading to Web Cache Deception and information disclosure. Affects Deno, Bun, and Node.js runtimes. Upgrade to hono >= 4.11.7.

## item #7 [security]
Dependency vulnerability: Hono arbitrary key read in Serve static Middleware (hono@< 4.11.7, GitHub Alert #3). Serve static Middleware for Cloudflare Workers adapter contains information disclosure vulnerability. Improper validation of user-controlled paths can result in unintended access to internal asset keys and arbitrary key reads from Workers environment. Only affects applications running on Cloudflare Workers with Serve static Middleware. Upgrade to hono >= 4.11.7.

## item #8 [bug]
Race condition in auth client initialization at src/services/google-auth.ts:63-86. The double-check locking pattern is broken. Between checks, two concurrent calls can both create separate GoogleAuth clients leading to multiple instances and potential memory leaks.
...
## item #10 [bug]
Race condition in file status cache at src/processing/storage/index.ts:149-228. updateFileStatus checks cache, reads from API on miss, then updates cache. Between check and update, another process could update same file causing stale cache entry and lost status updates.
...
## item #24 [edge-case]
Array index access without bounds check at src/services/document-sorter.ts:127,248,386. Direct array access after checking length === 0 but array could theoretically be empty between check and access in async contexts causing undefined access crashes.
...
## item #32 [edge-case]
extractDniFromCuit removes ALL leading zeros at src/utils/validation.ts:142-160. Line 157 uses replace(/^0+/, '') removing all leading zeros. If DNI portion is "00000000", returns empty string. Edge case for very low DNI numbers.
...
## item #35 [edge-case]
Memory leak potential in watch manager at src/services/watch-manager.ts:18-30. Module-level processedNotifications Map grows unbounded if channels never cleaned up. Cleanup only happens when new notifications arrive. Long-running servers accumulate notification history indefinitely.
...
## item #39 [edge-case]
Unbounded queue growth at src/processing/scanner.ts:229-232. processingPromises and retryPromises arrays grow unbounded. With many files, could cause memory exhaustion on large scans.

## item #40 [edge-case]
Memory leak in duplicate cache at src/processing/caches/duplicate-cache.ts:270-276. addEntry adds to cache after successful store but cache never cleared mid-scan. With thousands of files, cache grows unbounded until scan completes causing high memory usage.
