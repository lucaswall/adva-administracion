...
## item #1 [bug] [critical]
Race condition in lock acquisition at src/utils/concurrency.ts:100-123. TOCTOU vulnerability: between reading lock state (line 100) and setting it (line 105), another async task can acquire the same lock. Non-atomic check-and-set pattern violates mutual exclusion guarantees.

## item #2 [bug] [critical]
Lock state inconsistency at src/utils/concurrency.ts:104-114. The lock creation stores waitPromise AFTER creating it (line 114). If a waiter reads state between line 112 and 114, they get `waitPromise: undefined` and fall through to polling instead of proper wait.

## item #3 [bug] [high]
Race condition in pendingScan flag at src/processing/scanner.ts:365-384,711-714. Between checking `pendingScan` (line 365) and setting it (line 384), another concurrent invocation can check the flag and see `false`, causing both to proceed instead of one deferring.
