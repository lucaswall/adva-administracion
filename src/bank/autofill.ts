...
function parseRecibos(data: (string | number | boolean | null | undefined)[][]): Array<Recibo & { row: number }> {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    recibos.push({
      ...
      fileId: String(row[0] || ''),
      fileName: String(row[1] || ''),
      tipoRecibo: (row[2] || 'sueldo') as Recibo['tipoRecibo'],
      nombreEmpleado: String(row[3] || ''),
      cuilEmpleado: String(row[4] || ''),
      legajo: String(row[5] || ''),
      tareaDesempenada: row[6] ? String(row[6]) : undefined,
      cuitEmpleador: String(row[7] || ''),
      periodoAbonado: String(row[8] || ''),
      fechaPago: String(row[9] || ''),
