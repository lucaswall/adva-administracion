...
const MP_CHARGE_CONCEPTO_RE = /^MP \d+ - /;

/**
 * Checks if a concepto carries the synthetic `MP {id} - ` prefix written by the
 * MP sync. On MP accounts, every DEBIT row with this prefix is a commission or
 * tax-withholding charge generated by the sync — never a payment to a provider —
 * so it must be excluded from Egresos document matching.
 *
 * @param concepto - Bank transaction concept text
 * @returns True if the concepto has the MP operation prefix
 */
export function isMpChargeConcepto(concepto: string): boolean {
  return MP_CHARGE_CONCEPTO_RE.test(concepto || '');
}
...
export class BankMovementMatcher {
   ...
   * @param excludeFileIds - File IDs to exclude from matching
   * @param isMercadoPago - True when the movement belongs to an MP account (ADV-373 detection)
  ...
  matchMovement(
    ...
    // MP charge rows (commissions / tax withholdings) are synthetic debits
    // generated by the MP sync — they never correspond to an Egresos document,
    // and an amount/date coincidence must not AUTO-link one
    if (isMercadoPago && isMpChargeConcepto(movement.concepto)) {
  ...
  /**
   * Creates a match result for an MP charge debit row (commission / tax withholding)
   */
  private createMpChargeMatch(movement: MovimientoRow): BankMovementMatchResult {
    return {
      movement,
      matchType: 'bank_fee',
      description: 'Comisiones e impuestos Mercado Pago',
      matchedFileId: '',
      confidence: 'HIGH',
      reasons: ['MP charge row (commission/tax withholding)']
    };
  }
