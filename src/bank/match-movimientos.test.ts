...
describe('exchange rate prefetch', () => {
  ...
  it('should skip movimiento with MANUAL matchConfidence — not re-match it (Fix 5 - ADV-131)', async () => {
    const mockFolderStructure = {
      controlIngresosId: 'ingresos-id',
      controlEgresosId: 'egresos-id',
      bankSpreadsheets: new Map(),
    ...
    // MANUAL movimiento: already matched, locked by user
    vi.mocked(getMovimientosToFill).mockResolvedValue({
      ok: true,
      value: [
        {
          ...
          saldo: 9000,
          saldoCalculado: 9000,
          matchedFileId: 'existing-manual-match',
          detalle: 'Pago Manual a EMPRESA SA',
          matchConfidence: 'MANUAL' as const,
        },
      ],
    });

    // Matcher WOULD find a new match if not for the MANUAL guard
    mockMatchMovement.mockReturnValue({
      matchType: 'direct_factura',
      description: 'Pago Factura a EMPRESA SA',
      matchedFileId: 'new-factura-id',
      confidence: 'HIGH',
    });
    ...
    // MANUAL movimiento is skipped — updateDetalle called with empty array
    expect(updateDetalle).toHaveBeenCalledWith('bbva-id', []);

    // Matcher should NOT have been called for the MANUAL movimiento
