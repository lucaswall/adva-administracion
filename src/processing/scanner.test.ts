...
describe('scanner', () => {
  ...
  // Bug #3: retriedFileIds Map was at module level, causing shared retry counts across scans.
  // Fix: Moved retriedFileIds inside scanFolder function scope (scanner.ts).
  // Not testable via integration tests without inspecting internal state;
  // the fix is verified by code review and the absence of test interference in existing test suite.
